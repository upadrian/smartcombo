	...
	$.fn.smartCombo = function(method) {
		...
		var methods = {
			init: function(options) {
				...
				if ($(this).length == 1) methods.sc_init($(this), options)
			...
			getNode: function(JQOnode) {
				...
				if (JSOnode.tagName == "OPTGROUP") {
					...
					if (cache.props.multiple) html += '	<a href="javascript:;" class="' + cache.props._liClassOptGroupLabel + '" title="' + JQOnode.attr("label") + '">' + JQOnode.attr("label") + '</a>';
					else
			...
			setEvents: function() {
				if (cache.settings._openOnMouseEnter) cache.sc.bind('mouseenter', function() {
					cache.settings._open = methods.openWrapper();
				});
				if (cache.settings._closeOnMouseLeave && cache.settings._closeOnClick) cache.sc.bind('mouseleave', function() {
			...
			populateLabels: function() {
				...
				var label = tmp.join(", ");
				if (label == '') label = ((cache.props.multiple) ? cache.settings._text_labelMultiple : cache.settings._text_labelSimple);
				cache.sc_label.html(label);
			},
			reArmFromSource: function() {
				try {
					console.debug("rearmFromSource")
				} catch (e) {}
				...
				if (!cache.props.multiple && cache.settings._closeOnClick) cache.settings._open = methods.closeWrapper();
